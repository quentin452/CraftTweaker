/*
 * Copyright 2016-2019 Netflix, Inc. Licensed under the Apache License, Version 2.0 (the "License"); you may not use
 ...
 * <p>
 *
 * Instead of a long array to hold the bits, this implementation uses an AtomicLongArray, then does the appropriate
 * compare-and-swap operations when setting the bits.
 *
 * @author dkoszewnik
 *
 */
public class ThreadSafeBitSet {
    ...
    public ThreadSafeBitSet(int log2SegmentSizeInBits, int numBitsToPreallocate) {
        ...
        int numSegmentsToPreallocate = numBitsToPreallocate == 0 ? 1
    ...
    public void set(int position) {
        ...
        int longPosition = (position >>> 6) & segmentMask; /// which long in the segment -- remainder of div by num bits
                                                           /// per segment
    ...
    public int nextSetBit(int fromIndex) {
        ...
        int longPosition = (fromIndex >>> 6) & segmentMask; /// which long in the segment -- remainder of div by num
                                                            /// bits per segment
    ...
    /**
     * @return the number of bits which are current specified by this bit set. This is the maximum value to which you
     *         might need to iterate, if you were to iterate over all bits in this set.
     */
    ...
    /**
     * Return a new bit set which contains all bits which are contained in this bit set, and which are NOT contained in
     * the <code>other</code> bit set.
     * <p>
     *
    ...
    public ThreadSafeBitSet andNot(ThreadSafeBitSet other) {
        ...
        ThreadSafeBitSetSegments newSegments = new ThreadSafeBitSetSegments(
                thisSegments.numSegments(),
                numLongsPerSegment);
    ...
    private AtomicLongArray getSegment(int segmentIndex) {
        ...
        while (visibleSegments.numSegments() <= segmentIndex) {
            /// Thread safety: newVisibleSegments contains all of the segments from the currently visible segments, plus
            /// extra.
            /// all of the segments in the currently visible segments are canonical and will not change.
            ThreadSafeBitSetSegments newVisibleSegments = new ThreadSafeBitSetSegments(
                    visibleSegments,
                    segmentIndex + 1,
                    numLongsPerSegment);

            /// because we are using a compareAndSet, if this thread "wins the race" and successfully sets this
            ...
            if (segments.compareAndSet(visibleSegments, newVisibleSegments)) {
                ...
                /// then we will gather the new canonical sets from the update which we missed on the next iteration of
                /// this loop.
    ...
    private static class ThreadSafeBitSetSegments {
        ...
        private ThreadSafeBitSetSegments(int numSegments, int segmentLength) {
            ...
            /// Thread safety: Because this.segments is final, the preceding operations in this constructor are
            /// guaranteed to be visible to any
