...
/**
 * Contains an item stack. An item stack consists of an item definition, a meta or damage value and NBT data.
 * <p>
 * Item stacks can be retrieved using its name (or ID, in 1.6.4) with the bracket syntax. When using the bracket syntax,
 * by default, the item stack will match any size when used as ingredient. Item stacks can be multiplied with an integer
 * to create a stack of different size.
 * <p>
 * Item stacks are immutable. You can, however, easily create modified stacks using the helper methods. Adding
 * conditions and transformations will turn the item stack into an ingredients.
 *
 * @author Stan Hebben
 */
...
public interface IItemStack extends IIngredient {
    ...
    /**
     * Gets the display name. By default, it is the localized item name, but could also be the display name from NBT
     * data, or another item-specific name.
     *
    ...
    /**
     * Sets the display name. Only works if the name is translatable. Does not override names set by NBT data or using
     * item-specific logic.
     *
    ...
    /**
     * Gets the item tag. (NBT data) The resulting data is immutable and can only be changed with updateTag. Returns
     * empty data if there is no data. Never returns null.
     *
     * @return item tag
     */
    @ZenGetter("tag")
    ...
    /**
     * Gets the liquid contained in this item, if any. If this item stack is not an item container, it returns null.
     * Only returns the amount of liquid in a single item and doesn't into account stack size.
     *
     * @return liquid data
     */
    @ZenGetter("liquid")
    ...
    /**
     * Creates a weighted item stack with the given percentage chance. Does the same as item.weight(p * 0.01).
     *
    ...
    /**
     * Creates an item stack with the same item and stack size, but accepting any damage value. Only useful for items
     * used as ingredients.
     *
    ...
    /**
     * Retrieves all the ores referring to this item. Includes wildcard ore entries.
     *
