...
public interface IEventManager {
    ...
    /*
     * IEventHandle onPlayerUseItemStop(IEventHandler<PlayerUseItemStopEvent> ev); IEventHandle
     * onPlayerUseItemFinish(IPlayerUserItemFinishEventHandler ev); IEventHandle onPlayerChat(IPlayerChatEventHandler
     * ev); IEventHandle onTimerSingle(int millis, ITimerEventHandler ev); IEventHandle onTimerRepeat(int millis,
     * ITimerEventHandler ev); IEventHandle onEntityJoinWorld(IEntityJoinWorldEventHandler ev); IEventHandle
     * onEntityStruckByLightning(IEntityStruckByLightningEventHandler ev); IEventHandle
     * onLivingEnderTeleport(ILivingEnderTeleportEventHandler ev); IEventHandle
     * onLivingAttackEvent(ILivingAttackEventHandler ev); IEventHandle onLivingDeathEvent(ILivingDeathEventHandler ev);
     * IEventHandle onLivingJumpEvent(ILivingJumpEventHandler ev); IEventHandle
     * onLivingFallEvent(ILivingFallEventHandler ev); IEventHandle onLivingHurtEvent(ILivingHurtEventHandler ev);
     * IEventHandle onLivingDeathDropsEvent(ILivingDeathDropsEventHandler ev); IEventHandle
     * onItemTossed(IItemTossedEventHandler ev); IEventHandle onItemExpired(IItemExpiredEventHandler ev); IEventHandle
     * onMinecartCollision(IMinecartCollisionEventHandler ev); IEventHandle
     * onMinecartInteract(IMinecartInteractEventHandler ev); IEventHandle onCommand(ICommandEventHandler ev);
     */
}
