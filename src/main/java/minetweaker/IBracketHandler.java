 ...
 * 
 * Inside brackets, any kind of token is acceptable except the closing bracket. (&gt;) Bracket handlers (multiple
 * handlers can be registered) will resolve these tokens into actual values. Values have to be ZenScript symbols and
 * will resolve at compile-time.
 * 
 * These may of course return an expression that are executed on run-time, but the type of the resulting value must be
 * known compile-time.
 * 
 * @author Stan Hebben
 */
